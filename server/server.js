...
const io = new Server(server, socketOptions); // socket.io object
...
function Player(uuid, wheelInfo, scores, star = 0) {
  ...
  while (i < scoreRanks.length) {
    if (
      this.wheelInfo[options.cultures[i]] == "Prefer not to say" ||
      this.wheelInfo[options.cultures[i]] == "⊘"
    ) {
...
function PersonalReportData() {
  ...
  // music
  (this.music = "Jazz"), (this.nPlayerSameMusic = 0); // n "other" playeres also enjoy the same music
...
function initPuzzles() {
  ...
  let puzzleGroups = generateGroups(
    firstGroup,
    options.roomMaxPlayers[curLevel]
  );
  ...
  for (let i = 0; i < puzzleRooms.length; i++) {
    puzzleRooms[i].assignmentGenerator = new AssignmentGenerator(
      Object.keys(puzzleRooms[i].players)
    );
    puzzleRooms[i].freq = analyzeData(
      Object.keys(puzzleRooms[i].players),
      options.cultures
    );
  }
...
function generateReportData() {
  globalReportData.nPlayer = Object.keys(
    Object.assign({}, lobbyPlayers, goneLobbyPlayers)
  ).length;
  ...
  bigscreenReportData = options.cultures.reduce(
  ...
  let nPlayerLocationImportant = 0,
    nPlayerEthnicityImportant = 0;
  let importantLocationFreq = {},
    importantEthnicityFreq = {};
  ...
  for (let playerId in allPlayers) {
    ...
    locationFrequency[player.wheelInfo["home"]] =
      1 + (locationFrequency[player.wheelInfo["home"]] || 0);
    ethnicityFrequency[player.wheelInfo["ethnicity"]] =
      1 + (ethnicityFrequency[player.wheelInfo["ethnicity"]] || 0);
    ...
    if (player.scores[4] > 3)
      importantLocationFreq[player.wheelInfo["home"]] =
        (importantLocationFreq[player.wheelInfo["home"]] || 0) + 1;
    if (player.scores[5] > 3)
      importantEthnicityFreq[player.wheelInfo["ethnicity"]] =
        (importantEthnicityFreq[player.wheelInfo["ethnicity"]] || 0) + 1;

    for (let i = 0; i < 6; i++) {
      if (player.scores[i] > 3) {
        bigscreenReportData[options.cultures[i]][
          player.wheelInfo[options.cultures[i]]
        ] =
          1 +
          (bigscreenReportData[options.cultures[i]][
            player.wheelInfo[options.cultures[i]]
  ...
  locationFrequency["Prefer not to say"] =
    (locationFrequency["Prefer not to say"] || 0) +
    (locationFrequency["⊘"] || 0);
  delete locationFrequency["⊘"];
  if (locationFrequency["Prefer not to say"] == 0)
  ...
  for (let i = 0; i < 6; i++) {
    bigscreenReportData[options.cultures[i]]["Prefer not to say"] =
      (bigscreenReportData[options.cultures[i]]["Prefer not to say"] || 0) +
      (bigscreenReportData[options.cultures[i]]["⊘"] || 0);
    delete bigscreenReportData[options.cultures[i]]["⊘"];
    if (bigscreenReportData[options.cultures[i]]["Prefer not to say"] == 0)
      delete bigscreenReportData[options.cultures[i]]["Prefer not to say"];
  ...
  for (let playerId in allPlayers) {
    ...
    data.nMutualThumb = [...player.thumbsToPlayers].filter((x) =>
      player.thumbsFromPlayers.has(x)
    ).length;
    ...
    data.nPlayerLocationImportant =
      nPlayerLocationImportant - (data.locationIsImportant ? 1 : 0);
    ...
    data.importantLocationExamples = shuffle(
      data.importantLocationExamples
    ).slice(0, 3);

    data.ethnicityIsImportant = player.scores[5] > 3;
    data.nPlayerEthnicityImportant =
      nPlayerEthnicityImportant - (data.ethnicityIsImportant ? 1 : 0);

    let othersImportantEthnicityFreq = Object.assign(
      {},
      importantEthnicityFreq
    );
    ...
    data.nImportantEthnicities = Object.keys(
      othersImportantEthnicityFreq
    ).length;
    ...
    data.importantEthnicityExamples = shuffle(
      data.importantEthnicityExamples
    ).slice(0, 3);
    ...
    musicLocationTypes.splice(
      musicLocationTypes.indexOf("Prefer not to say"),
      1
    );
...
io.sockets.on("connection", function (socket) {
  socket.on("init", function (data) {
    ...
    if (socket.phase === 0) {
      ...
      if (socket.roomNumber < puzzleRooms.length) {
        ...
        let freq =
          data.uuid in firstPlayers
            ? analyzeData(Object.keys(activePlayers), options.cultures)
            : puzzleRooms[data.roomNumber].freq;
        let assignment = puzzleRooms[
          data.roomNumber
        ].assignmentGenerator.getAssignmentForPlayer(
          newPlayer.uuid,
          data.uuid in firstPlayers
    ...
    socket.on("disconnect", function () {
      if (socket.phase === 0) {
        ...
        if (socket.inBucket > -1) {
          ...
          if (
            players[socket.uuid].wheelInfo[
              puzzleOptions.cultures[socket.inBucket]
            ] === puzzleOptions.values[socket.inBucket]
          ) {
            console.log("wheelinfo matched");
            puzzleRooms[socket.roomNumber].realtimePoints[socket.inBucket] -= 1;
            if (
              puzzleOptions.expectedPlayerIds[socket.inBucket].includes(
                socket.uuid
              )
            ) {
              io.to("Room" + socket.roomNumber).emit(
                "subPoint",
                socket.inBucket,
                true
              );
  ...
  socket.on("startPuzzle", function () {
    ...
    for (let i = 0; i < puzzleRooms.length; i++) {
      ...
      for (let j = 0; j < idList.length; j++) {
        ...
        io.to(players[idList[j]].sid).emit(
          "startPuzzle",
          puzzleRooms[i].puzzleOptions,
          i,
          curLevel,
          options.levelTime[curLevel]
        );
      }
    }
    roomSolved = [];
  ...
  socket.on("bucketIn", (data) => {
    if (socket.roomNumber !== undefined) {
      ...
      if (
        players[socket.uuid].wheelInfo[puzzleOptions.cultures[data.index]] ===
        puzzleOptions.values[data.index]
      ) {
  ...
  socket.on("initReport", (uuid) => {
    ...
    if (
      gonePuzzlePlayers[uuid] !== undefined ||
      goneLobbyPlayers[uuid] !== undefined
    ) {
      console.log(`Player: ${uuid} is in gone pool`);
      let player = gonePuzzlePlayers[uuid]
        ? gonePuzzlePlayers[uuid]
        : goneLobbyPlayers[uuid];
  ...
  socket.on("initBigscreenReport", () => {
    ...
    let date = new Date();
    fs.writeFile(`${date.getTime()}.json`, data, err=>{
      if(err){
...
// broadcast players info 20 times per second 20 = 1000ms / 50
setInterval(() => {
  ...
  for (let i = 0; i < puzzleRooms.length; i++) {
    ...
    if (puzzleRooms[i].playerThumbUpInfo.size > 0) {
      io.to("Room" + i).emit(
        "thumbUp",
        Array.from(puzzleRooms[i].playerThumbUpInfo)
      );
